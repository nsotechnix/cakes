...
class Categorization extends CI_Controller
{
	...
	public function category()
	{
		...
		$data['CATEGORIES'] = $this->Crud->ciRead('categories', " `id` != '0'");
	...
	public function editCategory()
	{
		...
		if ($this->form_validation->run()) {
			...
			if ($isExists > 1) {
				...
				if ($this->Crud->ciUpdate('categories', $data, " `id` = '$categoryId'"))
	...
	public function subcategory()
	{
		...
		// $data['SUBCATEGORIES'] = $this->Admin_Product_Model->getsubcategory();
	...
	public function editSubCategory()
	{
		...
		if ($this->form_validation->run()) {
			...
			if ($isExists > 0) {
				...
				if ($this->Crud->ciUpdate('subcategories', $data, " `id` = '$subCategoryId'"))
	...
	public function update_subcategory()
	{
		...
		$this->load->model('Admin_Product_Model');
		$response = $this->Admin_Product_Model->update_subcategory($subcategory, $where);
		if ($response) {
	...
	public function addProduct()
	{
		...
		$data['CATEGORIES'] = $this->Admin_Product_Model->getcategory();
		$data['SUBCATEGORIES'] = $this->Admin_Product_Model->getsubcategory();
	...
	public function viewProduct()
	{
		...
		// $data['PRODUCTS'] = $this->Admin_Product_Model->select_products();
		$this->load->view('layouts/header');
		$this->load->view('layouts/bar');
		$this->load->view('layouts/sub-header');
		$this->load->view('layouts/nav');
	...
	public function editProduct_new($id)
	{
		...
		$data['PRODUCT_DETAILS'] = $this->Admin_Product_Model->getproduct_edit($id);
	...
	public function editThisProduct()
	{
		...
		if (isset($_POST['editProduct'])) {
			...
			if (isset($_FILES['productImage']) && is_uploaded_file($_FILES['productImage']['tmp_name'])) {
				$image_metadata = $this->upload->data();
				if (!$this->upload->do_upload('productImage')) {
					$this->session->set_flashdata('warning', 'Cover image error: ' . $this->upload->display_errors());
				} else {
					$configer =  array(
						'image_library' => 'gd2',
						'source_image' =>  $image_metadata['full_path'],
	...
	public function update_product()
	{
		...
		if ($this->form_validation->run()) {
			...
			$addproduct = new Admin_Product_Model;
	...
	public function block_product()
	{
		...
		$result = $this->Admin_Product_Model->productstatus([
		...
		if ($result) {
			$this->session->set_flashdata('product Suspended');
		}
	...
	public function unblock_product()
	{
		...
		if ($result) {
			$this->session->set_flashdata('product Reactivated');
		}
