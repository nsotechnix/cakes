...
class Workshop_Product_Model extends CI_Model
{
	...
	{
		$query = $this->db->select('stock_records.id');
		// $this->db->MAX("stock_records.current_stock");
		$this->db->from('stock_records');
		$this->db->group_by("stock_records.id");
		$this->db->order_by('stock_records.id');
		$this->db->get();
		return $query->result();
	}
