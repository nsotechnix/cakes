...
class Sale extends CI_Controller
{
    ...
    public function newSale()
    {
        ...
        $data['product'] = $this->Cashier_Sale_Model->get_product();

        $record = $this->Cashier_Sale_Model->get_sales_max();
        $ddd = $record->sales_no;
        $data['sales_no'] = $ddd + 1;
    ...
    public function viewStock()
    {
        ...
        $data['PRODUCTS'] = $this->Admin_Product_Model->getProducts();
        $this->load->view('layouts/header');
        $this->load->view('layouts/bar');
        $this->load->view('layouts/sub-header');
        $this->load->view('layouts/nav');
    ...
    {
        $data['sales'] = $this->Cashier_Sale_Model->select_sales();
        $this->load->view('layouts/header');
        $this->load->view('layouts/bar');
        $this->load->view('layouts/sub-header');
        $this->load->view('layouts/nav');
    ...
    public function editSale($id)
    {
        ...
        $data['sales_no'] = $ddd + 1;
        $data['product'] = $this->Cashier_Sale_Model->get_product();
        $data['saledetails'] = $this->Cashier_Sale_Model->fetchsaledetails($id);
        $data['saleproduct'] = $this->Cashier_Sale_Model->get_salesHistory($id);
        $this->load->view('layouts/header');
        $this->load->view('layouts/bar');
        $this->load->view('layouts/sub-header');
        $this->load->view('layouts/nav');
    ...
    public function saleInvoice($id)
    {
        ...
        $data['invoice'] = $this->Cashier_Sale_Model->get_saleInvoice($id);
        // $data['invoice'] = $this->Cashier_Sale_Model->get_salesHistory($id);
    ...
    public function pendingLedger()
    {
        ...
        $data['ledger'] = $this->Cashier_Sale_Model->select_pending_ledger($id);
        // $data['product'] = $this->Cashier_Sale_Model->get_productSales($sale_id);
        $this->load->view('layouts/header');
        $this->load->view('layouts/bar');
        $this->load->view('layouts/sub-header');
        $this->load->view('layouts/nav');
    ...
    public function completeLedger()
    {
        ...
        $id = $this->session->userdata('abCashierId');
        $sale_id = $this->input->post('sales_no');
        $data['ledger'] = $this->Cashier_Sale_Model->select_complete_ledger($id);
        // $data['product'] = $this->Cashier_Sale_Model->get_productSales($sale_id);
        $this->load->view('layouts/header');
        $this->load->view('layouts/bar');
        $this->load->view('layouts/sub-header');
    ...
    public function update_sales_ledger()
    {
        ...
        $response = $this->Cashier_Sale_Model->update_sales_ledger($ledger, $where);
        ...
        $response1 = $this->Cashier_Sale_Model->update_sales_ledger1($sales, $where1);
    ...
    public function saleReceipt($id)
    {
        ...
        $data['invoice'] = $this->Cashier_Sale_Model->get_ledgerReceipt($id);
    ...
    public function get_data_for_sale()
    {
        ...
        $data = $this->Cashier_Sale_Model->get_sales($product_id);
    ...
    function insert_sale()
    {
        ...
        for ($i = 0; $i < count($itemID); $i++) {
            ...
            if (strlen($unit_price[$i]) > 0) {
                $data1 = $this->Cashier_Sale_Model->check_stock_record($product_id[$i], $category_id[$i]);
                if ($data1) {

                    $data = $this->Cashier_Sale_Model->get_stock_qty($product_id[$i], $category_id[$i]);
                    $id = $data->stock;
                    ...
                    $this->Cashier_Sale_Model->update_record('products', $data, $where);
                } else {
                    $data = array(
                        "id" => $product_id[$i],
                    ...
                    $this->Cashier_Sale_Model->add_record('products', $data);
                }
            }

            $maxID = $this->Cashier_Sale_Model->get_sale_max1();
            $max = $maxID->sales_id;
            $pu_id = $max + 1;
            $sales_data = array(
                'sales_id' => $pu_id,
            ...
            $data_entry = $this->Cashier_Sale_Model->add_record('sales_detail', $sales_data);
        } //for loop
        ...
        $data_entry = $this->Cashier_Sale_Model->add_record('sales', $Sales_comp_Ins);
        ...
        $data_entry = $this->Cashier_Sale_Model->add_record('sales_ledger', $sales_ledger);
